...
def extract_frame(video, output, frame_config, video_config):
    ...
    while cap.isOpened():
        ...
        if ret:
            if not f % modulo:
                name = random_name(chars=frame_config["chars"], size=frame_config["size"], date=frame_config["date"])
                try:
                    greyscale = image_to_greyscale(frame)
                    resized = resize_image(greyscale, FRAME_TRANSFORMATIONS["ratios"])
                    cv.imwrite(os.path.join(output + os.sep, name + frame_config["ext"]), resized)
                    s += 1
